...
/** User call card when stats come from Telegram FaSol ingest (no McBot scanner labels). */
function createUserCallFaSolEmbed(scan, options = {}) {
  const tokenNameUpper = formatValue(scan?.tokenName, 'UNKNOWN TOKEN').toUpperCase();
  const tickerUpper = formatValue(scan?.ticker, 'UNKNOWN').toUpperCase();
  const ca = formatValue(scan?.contractAddress, 'Unknown');
    ...
    .setFooter({
      text: (
        options.footerText && String(options.footerText).trim()
          ? String(options.footerText).trim()
          : 'McGBot · User call'
      ).slice(0, 2048)
    })
    .setTimestamp();
